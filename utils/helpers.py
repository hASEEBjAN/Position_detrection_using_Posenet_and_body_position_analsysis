...
def display_position(image, image_draw, coordinates,image_font):
    """
    Display position or state of person (sit or stand)
    Args:
        image: PIL Image
            The loaded image the coordinate predictions are inferred for
        image_draw: PIL ImageDraw module
    ...
    coordinates_selected = [i for i in coordinates for j in range(len(concern_parts)) if i[0]==concern_parts[j]]

    P1 = [coordinates_selected[1][1], coordinates_selected[1][2]]
    P2 = [coordinates_selected[2][1], coordinates_selected[2][2]]
    P3 = [coordinates_selected[0][1], coordinates_selected[0][2]]

    angle_r = math.atan2(P3[1] - P1[1], P3[0] - P1[0]) - math.atan2(P2[1] - P1[1], P2[0] - P1[0])
    
    P1 = [coordinates_selected[4][1], coordinates_selected[4][2]]
    P2 = [coordinates_selected[5][1], coordinates_selected[5][2]]
    P3 = [coordinates_selected[3][1], coordinates_selected[3][2]]

    angle_l = math.atan2(P3[1] - P1[1], P3[0] - P1[0]) - math.atan2(P2[1] - P1[1], P2[0] - P1[0])
    if 180 + 0.1*180 >= abs(angle_r*60) >= 180 - 0.1*180 and 180 + 0.1*180 >= abs(angle_l*60) >= 180 - 0.1*180:
        print("Person is standing!!!")
        image_draw.multiline_text((10,10), "Standing", fill=(0, 0, 0), font=image_font)
    elif 270 + 0.1*270 >= abs(angle_r*60) >= 270 - 0.1*270 and 270 + 0.1*270 >= abs(angle_l*60) >= 270 - 0.1*270:
        print('Person is sitting!!!')
        image_draw.multiline_text((10,10), "Sitting", fill=(0, 0, 0), font=image_font)
    else:
        print(abs(angle_r*60), abs(angle_l*60))
        print('Unknow Position!!!')
        image_draw.multiline_text((10,10), "Unknown Position", fill=(0, 0, 0), font=image_font)
    return image
